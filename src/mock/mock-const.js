export const GENRES = [
  'Вестерн',
  'Триллер',
  'Семейный',
  'Боевик',
  'Исторический',
  'Мелодрама',
  'Фэнтези',
  'Приключения',
  'Драма',
  'Документальный',
];

export const DESCRIPTION = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras aliquet varius magna, non porta ligula feugiat eget. Fusce tristique felis at fermentum pharetra. Aliquam id orci ut lectus varius viverra. Nullam nunc ex, convallis sed finibus eget, sollicitudin eget ante. Phasellus eros mauris, condimentum sed nibh vitae, sodales efficitur ipsum. Sed blandit, eros vel aliquam faucibus, purus ex euismod diam, eu luctus nunc ante ut dui. Sed sed nisi sed augue convallis suscipit in sed felis. Aliquam erat volutpat. Nunc fermentum tortor ac porta dapibus. In rutrum ac purus sit amet tempus.';

export const POSTERS_DIR_PATH = './images/posters/';

export const POSTERS = [
  'made-for-each-other.png',
  'popeye-meets-sinbad.png',
  'sagebrush-trail.jpg',
  'santa-claus-conquers-the-martians.jpg',
  'the-dance-of-life.jpg',
  'the-great-flamarion.jpg',
  'the-man-with-the-golden-arm.jpg',
];

export const FILM_NAMES = new Map([
  ['Lord of The Rings', 'Fellowship of The Ring'],
  ['Джентльмены', 'The Gentlemen'],
  ['Шанхайский полдень', 'Shanghai Noon'],
  ['Расплата', 'The Accountant'],
  ['Короли улиц', 'Street Kings'],
  ['Пес-призрак: Путь самурая', 'Ghost Dog: The Way of the Samurai'],
  ['Леон', 'Léon'],
  ['Тайлер Рейк: Операция по спасению', 'Extraction'],
  ['Олдбой', 'Oldeuboi'],
  ['Тройная граница', 'Triple Frontier'],
  ['Дьявол всегда здесь', 'The Devil All the Time'],
  ['Поезд на Юму', '3:10 to Yuma'],
  ['Гладиатор', 'Gladiator'],
  ['Изгой-один: Звёздные войны. Истории', 'Rogue One'],
  ['Шрэк', 'Shrek'],
  ['Крестный отец', 'The Godfather'],
  ['Зеленая книга', 'Green Book'],
  ['Храброе сердце', 'Braveheart'],
]);

export const EMOTIONS = ['smile', 'sleeping', 'puke', 'angry'];

export const AGE_RATES = ['18+', '16+', '0+', '6+', '12+'];

export const FILM_DIRECTORS = [
  'Кристофер Нолан',
  'Стивен Спилберг',
  'Мартин Скорсезе',
  'Стэнли Кубрик',
  'Альфред Хичкок',
  'Акира Куросава',
  'Квентин Тарантино',
  'Хаяо Миядзаки',
  'Билли Уайлдер',
  'Фрэнсис Форд Коппола',
  'Питер Джексон',
  'Джеймс Кэмерон',
  'Вуди Аллен',
  'Клинт Иствуд',
  'Тим Бёртон',
  'Ридли Скотт',
  'Дэвид Финчер',
];

export const ACTORS = [
  'Алан Рикман',
  'Бенедикт Камбербэтч',
  'Бенисио Дель Торо',
  'Венсан Кассель',
  'Вигго Мортенсен',
  'Джеймс МакЭвой',
  'Джейк Джилленхол',
  'Дэниэл Дэй-Льюис',
  'Дэниэл Рэдклифф',
  'Бен Аффлек',
  'Оскар Айзек',
  'Киллиан Мёрфи',
  'Мадс Миккельсен',
  'Том Харди',
  'Хоакин Феникс',
  'Киану Ривз',
  'Педро Паскаль',
  'Джина Карано',
  'Форест Уитакер',
  'Орландо Блум',
];

export const WRITERS = [
  'Пол Шредер',
  'Оливер Стоун',
  'Аарон Соркин',
  'Уильям Голдман',
  'Чарли Кауфман',
  'Нора Эфрон',
  'Эрнест Леман',
  'Спайк Ли',
  'Джордж Лукас',
  'Престон Стёрджес',
  'Стэнли Кубрик',
  'Лоуренс Кэздан',
  'Стивен Зэйллян',
  'Орсон Уэллс',
  'Уэс Андерсон',
  'Джон Карпентер',
  'Ларс фон Триер',
  'Шейн Блэк',
  'Фрэнк Дарабонт',
  'Николас Пиледжи',
];

export const COMMENT_TEXTS = [
  'Interesting setting and a good cast',
  'Booooooooooring',
  'Lorem, ipsum dolor sit amet consectetur',
  'Nesciunt doloribus repellat saepe nobis blanditiis',
  'Doloribus, ullam ad! Labore, quaerat beatae?',
];

export const COMMENT_AUTHOR = [
  'Намджун',
  'Чонгук',
  'Чингачгук',
  'Гойко Митич',
  'Джин',
  'Юнги',
];

export const COUNTRIES = [
  'США',
  'Канада',
  'Новая Зеландия',
];
